<script lang="ts">
    ...
    import { addExpense, getExpenses } from "$lib/api/api";
	...
	onMount(async () => {
		try {
			...
			if (currentUserId) {
				try {
					const expenses = await getExpenses(currentUserId);
	...
	async function handleExpenseSubmit(event: CustomEvent<Expense>) {
		...
		try{
			...
			console.error("Failed to add new expenses to the server.", error);
		}
